...
class MaskSoftTeacher(MultiSteamDetector):
    ...
    def forward_train(self, img, img_metas, **kwargs):
        ...
        if "sup" in data_groups:
            ...
            log_every_n({
                "sup_gt_num":
                sum([len(bbox) for bbox in gt_bboxes]) / len(gt_bboxes)
            })
        ...
        if "unsup_student" in data_groups:
            unsup_loss = weighted_loss(
                self.foward_unsup_train(data_groups["unsup_teacher"],
                                        data_groups["unsup_student"]),
    ...
    def foward_unsup_train(self, teacher_data, student_data):
        ...
        with torch.no_grad():
            teacher_info = self.extract_teacher_info(
                teacher_data["img"][torch.Tensor(tidx).to(
                    teacher_data["img"].device).long()],
                [teacher_data["img_metas"][idx] for idx in tidx],
                [teacher_data["proposals"][idx] for idx in tidx] if
                ("proposals" in teacher_data) and
    ...
    def compute_pseudo_label_loss(self, student_info, teacher_info):
        M = self._get_trans_mat(teacher_info["transform_matrix"],
        ...
        if self.student.with_rpn:
            ...
            for bbox in pseudo_bboxes:
                bbox, _, _ = filter_invalid(
                    ...
                    score=
                    bbox[:,
                         4],  # TODO: replace with foreground score, here is classification score,
    ...
    def unsup_rcnn_cls_loss(
        ...
        **kwargs,
        ...
        loss["loss_cls"] = loss["loss_cls"].sum() / max(
            bbox_targets[1].sum(), 1.0)
    ...
    def extract_student_info(self, img, img_metas, proposals=None, **kwargs):
        ...
        student_info["transform_matrix"] = [
            torch.from_numpy(meta["transform_matrix"]).float().to(
                feat[0][0].device) for meta in img_metas
        ]
        return student_info
    ...
    def extract_teacher_info(self, img, img_metas, proposals=None, **kwargs):
        ...
        proposal_label_list = [
            p.to(feat[0].device) for p in proposal_label_list
        ]
        ...
        proposal_list, proposal_label_list, _ = list(
            zip(*[
                filter_invalid(
                    ...
                    thr=thr,
                    min_size=self.train_cfg.min_pseduo_box_size,
                ) for proposal, proposal_label in zip(proposal_list,
                                                      proposal_label_list)
            ]))
        det_bboxes = proposal_list
        reg_unc = self.compute_uncertainty_with_aug(feat, img_metas,
                                                    proposal_list,
                                                    proposal_label_list)
        det_bboxes = [
            torch.cat([bbox, unc], dim=-1)
        ...
        bbox_results = [
            bbox2result(det_bbox, det_label,
                        self.teacher.roi_head.bbox_head.num_classes)
            for det_bbox, det_label in zip(det_bboxes, det_labels)
        ]
        det_bboxes, det_labels = multi_apply(result2bbox, bbox_results)
        det_bboxes = [
            torch.from_numpy(bbox).float().to(feat[0].device)
    ...
    def compute_uncertainty_with_aug(self, feat, img_metas, proposal_list,
                                     proposal_label_list):
        auged_proposal_list = self.aug_box(proposal_list,
                                           self.train_cfg.jitter_times,
                                           self.train_cfg.jitter_scale)
        # flatten
        auged_proposal_list = [
        ...
        bboxes = [
            ...
            if bbox.numel() > 0 else bbox.new_zeros(
                self.train_cfg.jitter_times, 0, 4 * reg_channel).float()
            for bbox in bboxes
        ]
    ...
    def aug_box(boxes, times=1, frac=0.06):
        def _aug_single(box):
            ...
            box_scale = (box_scale.clamp(min=1)[:, None, :].expand(-1, 2,
                                                                   2).reshape(
                                                                       -1, 4))
            aug_scale = box_scale * frac  # [n,4]

            offset = (torch.randn(times, box.shape[0], 4, device=box.device) *
                      aug_scale[None, ...])
